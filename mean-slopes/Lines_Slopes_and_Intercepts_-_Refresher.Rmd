...
# a fucntion for a plot showing the origin
...
![](origin.gif) 
<br>
<center>(Image from https://www.varsitytutors.com/hotmath/hotmath_help/topics/graphing-scale-and-origin)<center>
<br>
...
You'll notice that the point `(x = 5, y = 5)` is on this line. A line can be thought of as a collection of infinitely close points. The equation for a line tells you which points are on the line. We can refer to the line on the graph above as `y = x`. 
...
The scatterplot below shows the `Clarity` ratings and the `Overall Quality` ratings for each course (shown in the `Discipline` column).
...
plt.scatter(clarity, quality)
plt.xlabel('Calrity Rating')
plt.ylabel('Overall Quality Rating');
```

We can fit a line to this data by using mathematical or computational techniques which find the equation for the line which minimizes the collective vertical distance between the datapoints and the line (we will go into more detail on these techniques [elsewhere](finding_lines)).
...
Remember that all lines pass through the origin if they do not have a nonzero intercept (i.e. an intercept that is any other number apart from 0, in the case of the scatterplot above it is 0.32). 
...
plt.plot(3.6, predict, 'X', markersize = 15, color = 'red', label = 'Predicted "Overall Quality" rating for "Clarity == 3.6"' )
zer_to_pred = np.linspace(0,predict)
zer_to_3_point_6 = np.linspace(0, 3.6, num = len(zer_to_pred))
plt.plot(np.repeat(3.6, len(zer_to_pred)), zer_to_pred, '--', color = 'darkred')
plt.plot(zer_to_3_point_6, np.repeat(predict, len(zer_to_3_point_6)),'--', color = 'darkred')
plt.legend(bbox_to_anchor = (1,1))
plt.scatter(clarity, quality);
```
